...
#include <vector>
#include "tms_if_for_opera/Excavator/excavator_follow_waypoints.hpp"
// #include <glog/logging.h>

using std::placeholders::_1;
using std::placeholders::_2;

ExcavatorFollowWaypoints::ExcavatorFollowWaypoints() : rclcpp::Node("tms_if_follow_waypoints_node")
{
    this->action_server_ = rclcpp_action::create_server<FollowWaypoints>(
        this, "tms_rp_navigate_follow_waypoints",
        std::bind(&ExcavatorFollowWaypoints::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
        std::bind(&ExcavatorFollowWaypoints::handle_cancel, this, std::placeholders::_1),
        std::bind(&ExcavatorFollowWaypoints::handle_accepted, this, std::placeholders::_1));

    
    action_client_ = rclcpp_action::create_client<FollowWaypoints>(this, "follow_waypoints");
}

rclcpp_action::GoalResponse ExcavatorFollowWaypoints::handle_goal(
...
rclcpp_action::CancelResponse ExcavatorFollowWaypoints::handle_cancel(const std::shared_ptr<GoalHandle> goal_handle)
{
...
void ExcavatorFollowWaypoints::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle)
{
    using namespace std::placeholders;
    std::thread{ std::bind(&ExcavatorFollowWaypoints::execute, this, _1), goal_handle }.detach();
}

void ExcavatorFollowWaypoints::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
...
void ExcavatorFollowWaypoints::goal_response_callback(const GoalHandleFollowWaypoints::SharedPtr& goal_handle)
{
  if (!goal_handle)
  {
...
void ExcavatorFollowWaypoints::feedback_callback(
...
//result
void ExcavatorFollowWaypoints::result_callback(const std::shared_ptr<GoalHandle> goal_handle,
...
int main(int argc, char* argv[])
{
    ...
    rclcpp::spin(std::make_shared<ExcavatorFollowWaypoints>());
    rclcpp::shutdown();
    return 0;
}
