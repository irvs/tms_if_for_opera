...
#include <vector>
#include "tms_if_for_opera/Excavator/excavator_navigate_through_poses.hpp"
// #include <glog/logging.h>

using std::placeholders::_1;
using std::placeholders::_2;

ExcavatorNavigateThroughPoses::ExcavatorNavigateThroughPoses() : rclcpp::Node("tms_if_navigate_through_poses_node")
{
    this->action_server_ = rclcpp_action::create_server<NavigateThroughPoses>(
        this, "tms_rp_navigate_through_poses",
        std::bind(&ExcavatorNavigateThroughPoses::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
        std::bind(&ExcavatorNavigateThroughPoses::handle_cancel, this, std::placeholders::_1),
        std::bind(&ExcavatorNavigateThroughPoses::handle_accepted, this, std::placeholders::_1));

    
    action_client_ = rclcpp_action::create_client<NavigateThroughPoses>(this, "navigate_through_poses");
}

rclcpp_action::GoalResponse ExcavatorNavigateThroughPoses::handle_goal(
...
rclcpp_action::CancelResponse ExcavatorNavigateThroughPoses::handle_cancel(const std::shared_ptr<GoalHandle> goal_handle)
{
...
void ExcavatorNavigateThroughPoses::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle)
{
    using namespace std::placeholders;
    std::thread{ std::bind(&ExcavatorNavigateThroughPoses::execute, this, _1), goal_handle }.detach();
}

void ExcavatorNavigateThroughPoses::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
...
void ExcavatorNavigateThroughPoses::goal_response_callback(const GoalHandleNavigateThroughPoses::SharedPtr& goal_handle)
{
  if (!goal_handle)
  {
...
void ExcavatorNavigateThroughPoses::feedback_callback(
...
//result
void ExcavatorNavigateThroughPoses::result_callback(const std::shared_ptr<GoalHandle> goal_handle,
...
int main(int argc, char* argv[])
{
    ...
    rclcpp::spin(std::make_shared<ExcavatorNavigateThroughPoses>());
    rclcpp::shutdown();
    return 0;
}
