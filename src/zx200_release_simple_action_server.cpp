...
void Zx200ReleaseSimpleActionServer::execute(const std::shared_ptr<GoalHandleZx200ReleaseSimple> goal_handle)
{
  ...
  target_joint_values = joint_values;
  // target_joint_values["bucket_joint"] += goal->target_angle / 2.0;

  for (size_t i = 0; i < joint_names_.size(); i++) {
    if (joint_names_[i] == "bucket_joint") {
      target_joint_values[i] = (joint_values[i] + goal->target_angle) / 2.0;  // バケットの目標角度を設定1
    }
  }
  ...
  for (size_t i = 0; i < joint_names_.size(); i++) {
    if (joint_names_[i] == "arm_joint") {
      ...
      target_joint_values[i] = goal->target_angle;
